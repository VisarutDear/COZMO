...
import http.client
import asyncio
from LINEnoti import linenotify
...
def after_alarm(robot: cozmo.robot.Robot):
    ...
    while True :
        ...
        if face_to_follow:
            # start turning towards the face
            print("found")
            robot.say_text("Hey! DEAR it's time to take medicine!, You have to take"+str(len(alarm_med))+"pills").wait_for_completed()
            # turn_action = robot.turn_towards_face(face_to_follow)
            for k in alarm_med :
                robot.say_text("Take" + ' ' + k['name']  + ' ' + "for" + k['quantity']+ "tablet").wait_for_completed()
            print("work")
            robot.say_text("I will repeat agian" + ' ' + "please listen carefully").wait_for_completed()
            for k in alarm_med :
                robot.say_text("Take" + ' ' + k['name']  + ' ' + "for" + k['quantity']+ "tablet").wait_for_completed()
            return
        ...
        if not (face_to_follow and face_to_follow.is_visible):
            ...
            # find a visible face, timeout if nothing found after a short while
            try:
                face_to_follow = robot.world.wait_for_observed_face(timeout=20)
            except asyncio.TimeoutError:
                linenotify()
                print("Didn't find a face - exiting!")
                return
