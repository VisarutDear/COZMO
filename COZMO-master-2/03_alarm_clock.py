    ...
    while True:
        ...
        if do_alarm:
            ...
            with robot.perform_off_charger():
                ...
                robot.say_text("Hey! DEAR it's time to take medicine! now the time is" + short_time_string).wait_for_completed()
                #robot.drive_straight(distance_mm(100),speed_mmps(100))
                #robot.move_lift(3)
                #robot.say_text("med time med time" + short_time_string).wait_for_completed()
        else:
